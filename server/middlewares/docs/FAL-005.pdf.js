...
async function FAL005(orden, solicitud, Lote, materiales, lotes, cantidades, Requi, limpieza, email, usuario) {
    ...
    if (orden != 'N/A') {
        currentYear = orden.substring(0, 4)
    }
        ...
        if (fs.existsSync(directoryPath)) {
            ...
            if (matchingFolder) {
                ...
                if (!fs.existsSync(path_new)) {
                    ...
                    const almacenado_ = pdf;
                    almacenado_.pipe(fs.createWriteStream(`${path_new}\\AL-ASG-${solicitud}_${orden}_${formattedDateTime}.pdf`));
                    pdfPath = `${path_new}\\AL-ASG-${solicitud}_${orden}_${formattedDateTime}.pdf`;
                    console.log(`Found folder starting with order number ${orden}: ${matchingFolder}`);
                }
    ...
    if (Requi) {
        asignacion_(orden, solicitud, Lote, pdfStream, 'Equipo', `calcurianandres@gmail.com,yraida.baptista@poligraficaindustrial.com,zuleima.vela@poligraficaindustrial.com,jaime.sanjuan@poligraficaindustrial.com,${email}`)
        ...
        asignacion(orden, solicitud, Lote, pdfStream, 'Equipo', `calcurianandres@gmail.com,yraida.baptista@poligraficaindustrial.com,zuleima.vela@poligraficaindustrial.com,jaime.sanjuan@poligraficaindustrial.com,${email}`)
    }
