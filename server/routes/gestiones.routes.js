...
app.get('/api/estadisticas/impresoras', (req, res) => {
    ...
    Gestion.find({ fecha: { $regex: /^2025/ }, maquina: '620d53d1f53de725bcfbe8f2' })
        .populate('op')
        .exec((err, gestionDb) => {
            if (err) {
                return res.status(400).json({
            ...
            Gestion.find({ fecha: { $regex: /^2025/ }, maquina: '620d53eaf53de725bcfbe8f3' })
                .populate('op')
                .exec((err, gestionDb) => {
                    if (err) {
                        return res.status(400).json({
                    ...
                    Gestion.find({ fecha: { $regex: /^2025/ }, maquina: '620d5422f53de725bcfbe8f4' })
                        .populate('op')
                        .exec((err, gestionDb) => {
                            if (err) {
                                return res.status(400).json({
